describe('Hospital', () => {
  ...
  it('should display state and city dropdowns', () => {
    cy.intercept('GET', 'https://meddata-backend.onrender.com/states', { fixture: 'states.json' }).as('getStates');
    cy.wait('@getStates');
    cy.get('#state').should('be.visible'); // Verify the State dropdown is present
    cy.get('#city').should('be.visible'); // Verify the City dropdown is present
  });

  it('should fetch hospitals when both state and city are selected', () => {
    cy.intercept('GET', 'https://meddata-backend.onrender.com/states', { fixture: 'states.json' }).as('getStates');
    cy.intercept('GET', 'https://meddata-backend.onrender.com/cities/Alabama', { fixture: 'cities.json' }).as('getCities');
  ...
  it('should open appointment booking section when a medical center is clicked', () => {
    ...
    cy.get('#state') // Replace with the selector for the state dropdown
    .click();
    cy.wait('@getStates');
